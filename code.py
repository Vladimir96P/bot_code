...
#     db_obj.execute('SELECT user_id FROM bot_users_list')
#     print(db_obj.fetchall())
#     id_list = db_obj.fetchall()
#     for i in id_list:
#         bot.send_message(int(i[0]), 'Приветик!😜 У меня немного изменились рекомендации по калорийности, касательно похудения. Если худеешь - вызови еще раз кнопку сжечь жирок 🥦. Возможно именно для тебя были сделаны изменения! Желаю хорошей недели 😊')
